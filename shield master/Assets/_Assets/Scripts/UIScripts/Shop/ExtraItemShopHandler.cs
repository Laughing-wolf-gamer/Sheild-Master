    ...
    public class ExtraItemShopHandler : MonoBehaviour {
        ...
        public void TryBuyAbility(){
            if(abilityItemArray[currentIndex].CanBuyItem(playerDataSO.GetCashAmount(),playerDataSO.GetDimondCount(),2)){
                ...
                AnayltyicsManager.current.SpendVirualCurrency(abilityItemArray[currentIndex].GetAbilityCost().coinCount + abilityItemArray[currentIndex].GetAbilityCost().coinCount
                    ,abilityItemArray[currentIndex].name,string.Concat("Cash ",abilityItemArray[currentIndex].GetAbilityCost().coinCount ," Dimonds " ,
                     abilityItemArray[currentIndex].GetAbilityCost().dimondCount)
                );
